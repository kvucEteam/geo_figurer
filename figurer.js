...
function setEventListeners() {
    ...
    $(document).on('click', ".generalInfo", function(event) {
        ...
        UserMsgBox('body', "<h3>Hvad er en figur?</h3><p>En figur er en <b>grafisk fremstilling af data</b>, der gør det lettere at <b>overskue en udvikling eller en sammenhæng</b>.</p><br><p>Den figurtype der vælges til at afbillede data afhænger af <ul><li> tradition </li><li>modtageren de er tænkt til </li><li>og selvfølgelig den figurtype der egner sig bedst til netop den datatype.</li></ul></p><b>Enhederne</b> fortæller hvad der <b>vises</b> eller anskueliggøres. </p><p>Det kan være alt fra CO<sub>2</sub>-udledning pr. indbygger til gennemsnitstemperaturer eller befolkningens fordeling i aldersgrupper. Enhederne på en figur skal passe til dataens størrelser, så det er muligt at se eventuelle variationer. </p><br/><p>Der er ingen faste regler eller konventioner om <b>hvad forskellige farver betyder</b>, se derfor altid hvad der står i <b>signaturforklaringen.</b> </p><p>Signaturer kan være farver men også <b>tegn eller ikoner</b>, som har en særlig betydning. Fælles for signaturer er, at de er fremhævede, så det netop er dem man skal have fokus på.</p><h3>Hvordan anvender man en figur?</h3><ol><li><b>Start altid med OVERSKRIFTEN</b>. Den fortæller hvad figuren beskriver.</li><li><b>Identificer FIGURTYPEN</b>. Den afgør hvordan man aflæser data, og om der er en eller flere variable i spil.</li><li><b>Se på hvilke ENHEDER eller SIGNATURER der bruges på figuren</b>. De fortæller hvad der undersøges og skalaen der undersøges i. </li><li><b>Identificer og beskriv FIGURENS INDHOLD </b>.  Angiv f.eks. en tendens med konkrete aflæsninger. Dette er det redegørende niveau.</li><li><b>Forklar FIGURENS INDHOLD</b>. Her skal teorien kobles på. Sammenlign, forklar, bedøm og argumenter for hvorfor indholdet ser ud som det gør. Dette er det analyserende niveau.</li>"); // Redigeret ATO juli 2017
        $("body").click(function() {


                $("body").off();
                if (start == 0) {
                    microhint($("#DataInput"), "Vælg hvilket diagram du vil arbejde med", true);
                    microhint($(".glyphicon-chevron-right"), "Klik og gå til første opgave", true);
                    //microhint($(".img-responsive"), "Orienter dig i tabellen", true);
...
function actions_answerCorrect(carouselNo, slideNo) {
    var mh_append = $(".carouselPage").eq(carouselNo).find(".answerWrap").eq(slideNo - 1);
    ...
    if (typeof(feedbackObj) !== 'undefined') { // Only if the teachers have supplied specific feedback for this studentanswer, then...
        microhint(mh_append, '<div class="microhint_label_success">Du har svaret korrekt</div><p>' + feedbackObj.correct + '</p>');
    } else { // ... else give the student the general answer. 
        microhint(mh_append, '<div class="microhint_label_success">Du har svaret korrekt</div><p>' + jsonData.slideData[carouselNo].carouselData.feedback.correct + '</p>');
    }
}

function actions_answerWrong(carouselNo, slideNo) {
    var mh_append = $(".carouselPage").eq(carouselNo).find(".answerWrap").eq(slideNo - 1);
    //cplength.hide(); // .css("color", "red");
    ...
    if (typeof(feedbackObj) !== 'undefined') { // Only if the teachers have supplied specific feedback for this specific studentanswer, then...
        ...
        if (Array.isArray(feedbackObj.wrong)) { // If an array of feedback posibilities is present, then...   // <---------  ADDED 21-11-2016
            ...
            microhint(mh_append, '<div class="microhint_label_danger">Du har svaret forkert</div><p>' + feedbackObj.wrong[ansIndex] + '</p>');
        } else {
            microhint(mh_append, '<div class="microhint_label_danger">Du har svaret forkert</div><p>' + feedbackObj.wrong + '</p>');
        }
    } else { // ... else give the student the general answer. 
        microhint(mh_append, '<div class="microhint_label_danger">Du har svaret forkert</div><p>' + jsonData.slideData[carouselNo].carouselData.feedback.wrong + '</p>');
    }
}
