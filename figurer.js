...
function initRadioAndCheckboxes() {
    $(".answerWrap").each(function(index, element) {
        ...
        /*----------  Randomisering udkommenteret - kræver at vi laver logikken om i checksvar (ATO 15 dec. 2016)  ----------*/
        $(this).find('.answerChoice').shuffle_div_position();
    });
}
...
function checkAnswer(carouselNo, slideNo) {
    ...
    switch (quizObj.type) {
        ...
        case "radio":
            ...
            //var checked_val = $('#carouselId_' + carouselNo + ' #slide_' + slideNo + ' input').
            
            /*----------  Jeg har ændret en smule i logikken, for at kunne shuffle radio buttons  ----------*/


            var checked_val = $('#carouselId_' + carouselNo + ' #slide_' + slideNo + ' input:checked').val();
            //console.log("checked_val: " + checked_val);
            

            if (checked_val == answerNo) {
                actions_answerCorrect(carouselNo, slideNo);
            } else {
                actions_answerWrong(carouselNo, slideNo);
            }
